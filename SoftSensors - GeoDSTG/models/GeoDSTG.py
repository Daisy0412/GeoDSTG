...
# Simple Transformer Block for ablation experiments
...
# GAGB
class WeightedSAGEConvWithBias(MessagePassing):
...
class GeoDSTG(nn.Module):
    def __init__(self, history_steps, out_channel=1, hidden_size=64,
        ...
        self.graph_learner = GraphLearner(input_dim=64, num_nodes=num_nodes)

        self.temporal_layer1 = ConformerBlock(hidden_size, num_heads, dropout)
        self.temporal_layer2 = ConformerBlock(hidden_size, num_heads, dropout)

        # Spatial module
        self.spatial_static1 = WeightedSAGEConvWithBias(hidden_size, hidden_size, pos_dim=hidden_size)
        self.spatial_static2 = WeightedSAGEConvWithBias(hidden_size, hidden_size, pos_dim=hidden_size)
